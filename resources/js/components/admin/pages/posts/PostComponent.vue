						...
						<table class="table card-table table-vcenter text-nowrap table-nowrap">
							<thead  class="blue-background text-white">
								<tr>
									...
									<th class="text-white w-15">Tiêu đề</th>
									<th class="text-white w-30">Tác giả</th>
									<th class="text-white w-30">Ngày đăng</th>
							...
							<tbody>
								<tr v-show="posts.length" v-for="post in posts" :key="post.post_id">
									...
									<td @click="detail(post)"><a href="javascript:void(0)">{{ post.post_title }}</a></td>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// },
			// change(post_id) {
			// 	axios.patch(`../../api/admin/edu-major/chuyen-nganh/change/${post_id}`)
			// 	.then(res => {
			// 		this.fetchPosts();
			...
			// 	let vm = this;
			// 	page_url = `../../api/admin/edu-major/chuyen-nganh/detail/${major.post_id}`;
			// 	fetch(page_url)
			// 	.then(res => res.json())
			// 	.then(res => {
