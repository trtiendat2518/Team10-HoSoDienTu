    ...
    <div>
        ...
        <div class="page-header">
            <ol class="breadcrumb"><!-- breadcrumb -->
                ...
                <li class="breadcrumb-item">
                    <router-link tag="a" :to="{ name: 'classstudentindex' }">Danh sách lớp học</router-link>
                </li>
                <li class="breadcrumb-item active" aria-current="page">Danh sách sinh viên</li>
            </ol><!-- End breadcrumb -->
        </div>

        <router-link class="btn btn-primary btn-lg mb-3 btn-3d" tag="button" :to="{ name: 'classstudentindex' }">
            ...
            <div class="col-md-12 col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <div class="col-md-6">
                            <h2 class="card-title">
                                ...
                                <div v-else>
                                    <b>Danh sách sinh viên lớp {{ course_code }}-{{ class_name }}</b>
                                </div>
                            </h2>
                        </div>
                        <div class="col-md-6">
                    ...
                    <div class="ml-2 mt-3">
                        ...
                        <div v-else>
                            <h4>Chủ nhiệm sinh viên: <i>{{ lecturer_fullname }}</i></h4>
                        </div>
                    </div>
                            ...
                            <tbody>
                                <tr v-show="students.length" v-for="stu in students" :key="stu.student_id">
                                    ...
                                    <td>
                                        ...
                                        @click="show(clas)"></button>
                                    </td>
                                    <td>
                                        <button class="btn-3d btn btn-danger btn-lg fa fa-trash"
                                        @click="destroy(clas.class_id)"></button>
                                    </td>
                                </tr>
...
<script>
    export default {
        ...
        methods: {
            fetchStudentClass(class_id, page_url) {
                let vm = this;
                class_id = this.class_id;
                page_url = `../../api/admin/class-sv/lop/student-class/${class_id}`;
                fetch(page_url)
                .then(res => res.json())
                .then(res => {
            ...
            fetchLecturers(class_id, page_url) {
                let vm = this;
                class_id = this.class_id;
                page_url = `../../api/admin/class-sv/lop/teacher-student-class/${class_id}`;
                fetch(page_url)
                .then(res => res.json())
                .then(res => {
                    if (res.data.length>0) {
            ...
            search(class_id, page_url) {
                let vm = this;
                class_id = this.class_id;
                page_url = `../../api/admin/class-sv/lop/search-student-class/${class_id}/${this.query}`;
                fetch(page_url)
                .then(res => res.json())
                .then(res => {
                    this.students = res.data;
